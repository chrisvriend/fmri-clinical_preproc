...
def denoise(img_file, tsv_file, out_path, col_names=False, hp_filter=False, lp_filter=False, out_figure_path=False):
    ...
    if lp_filter:
        ...
        results.residuals = butterworth(results.residuals,
                                        sampling_rate=Fs,
                                        low_pass=low_pass, high_pass=None)
        print('Low-pass Filter Applied: < ' + str(low_pass) + ' Hz')

    # add mean back into data
    clean_data = results.residuals.T + np.reshape(data_mean, (Nvox, 1))
    ...
    # #changed from reporting to plotting
    ...
    # # FD timeseries plot
    # FD = 'FD'
    # poss_names = ['framewise_displacement', FD, 'framewise_displacement', 'fd']
